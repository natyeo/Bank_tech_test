...
describe Statement do
  ...
  describe '#print' do
    it 'prints "16/12/2019 || 1000.00 || || 1000.00" when 1000 deposited' do
      ...
      expect(statement.print_statement).to include "#{Time.now.strftime('%d/%m/%Y')} || 1000.00 || || 1000.00"
    end
    ...
    it 'prints account statement newest entry first' do
      ...
      expect(statement.print_statement).to include "#{Time.now.strftime('%d/%m/%Y')} || || 500.00 || 1500.00\n#{Time.now.strftime('%d/%m/%Y')} || 2000.00 || || 2000.00"
    end
